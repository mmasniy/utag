...
void MainTable::setTable(const QString& sPath) {
    ...
    for (const auto& i : m_application.getTagsInfo()) {
        ...
        setItem(rowCount() - 1, 1, fileName);
        setItem(rowCount() - 1, 2, new QTableWidgetItem(i.getAllInfo()["title"]));
        setItem(rowCount() - 1, 3, new QTableWidgetItem(i.getAllInfo()["artist"]));
        setItem(rowCount() - 1, 4, new QTableWidgetItem(i.getAllInfo()["album"]));
        setItem(rowCount() - 1, 5, new QTableWidgetItem(i.getAllInfo()["track_number"]));
        setItem(rowCount() - 1, 6, new QTableWidgetItem(i.getAllInfo()["genre"]));
        setItem(rowCount() - 1, 7, year);
        setItem(rowCount() - 1, 8, absPath);
    }
}
